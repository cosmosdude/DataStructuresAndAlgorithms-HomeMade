#include "../../Heap/MinHeap/MinHeap.h"
...
template<typename T>
void LowPriorityQueue<T>::enqueue(int priority, const T& element) noexcept {
	heap.push(priority, element);
}

template<typename T>
void LowPriorityQueue<T>::dequeue() {
	throw_if_empty("Unable to dequeue empty priority queue.");
	heap.pop();
}

template<typename T>
T LowPriorityQueue<T>::peek() const {
	throw_if_empty("Unable to peek empty priority queue.");
	return heap.peek();
}

template<typename T>
size_t LowPriorityQueue<T>::size() const noexcept {
	return heap.size();
}

template<typename T>
bool LowPriorityQueue<T>::is_empty() const noexcept {
